...
In order to achieve the aim of predicting creditability, we will take into account 7 variables from the "germancredit" data and determine which one has the most influence on the precision of a predicted desired value. Considering the Filtering of variables with \>= iv_limit = 0.02, \<= missing_limit = 0.95 and \<= identical_limit_limit = 0.95.
...
```{r}
data_woe.list = lapply(data_f.list, function(x) woebin_ply(x, bins.list)) 
lapply(data_woe.list, class) 
lapply(data_woe.list, dim)
```

```{r}
data_grp.list = lapply(data_f.list, function(x) 
woebin_ply(x, bins.list, to = 'bin')) 
lapply(data_grp.list, class) 
lapply(data_grp.list, dim) ## Generalized linear model (glm): Regressing response w.r.t. predictors
```
